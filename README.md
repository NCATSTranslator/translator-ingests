...
8. **Supplemental Ingest Documentation** **_(as needed)_**: if helpful but ad hoc supplemental documentation is generated and needs to be retained without too much lint (codespell, ruff) scrutiny, then it can be stored for future reference within an ingest-specific subdirectory within the [ingest docs](https://github.com/NCATSTranslator/translator-ingests/tree/main/src/docs/ingests) directory. Unlike the ingest RIG, the format and content isn't standardized but should be self-documenting (in its own folder README)
