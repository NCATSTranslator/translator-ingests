"""SemMedDB uncapped ingest: KG2 pre-processed edges -> Biolink Model associations.

Identical to the standard SemMedDB ingest but with publication capping disabled,
so all PMIDs are retained on every edge regardless of count.
"""
...
# Publication capping is DISABLED in this variant -- all PMIDs are retained.
PUBLICATIONS_CAP_ENABLED: bool = False
...
def on_end_filter_edges(koza: koza.KozaTransform) -> None:
    ...
    s = koza.state
    koza.log("semmeddb_uncapped processing complete:", level="INFO")
...
def transform_semmeddb_edge(
    ...
    # No publication capping in this variant -- all PMIDs retained.
