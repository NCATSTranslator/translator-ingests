...
|  MacromolecularComplex | contributes_to |  MolecularFunction | n/a | varies, depending on mappings from GO evidence codes   | 'negated', 'has_evidence', 'publications' | A GO Annotation uses 'contributes_to' predicate when a gene product is required as part of a macromolecular complex for executing the reported function. |
...
|  Gene, Protein, MacromolecularComplex, RNAProduct  | colocalizes_with |  CellularComponent | n/a | varies, depending on mappings from GO evidence codes   | 'negated', 'has_evidence', 'publications' |  A GO Annotation uses 'colocalizes_with' predicate when a gene product has a transient or dynamic association with the reported cellular component.|
