...
from translator_ingest.util.biolink import INFORES_GOA
...
def transform_record(koza: koza.KozaTransform, record: dict[str, Any]) -> Iterable[Any]:
    ...
    go_term = go_biolink_class(
        id=go_id, category=go_biolink_class.model_fields["category"].default  # Dynamic category from Biolink model
    )
    ...
    if not predicate:
        ...
        if not predicate:
            ...
            koza.log(
                msg="Using fallback predicate for " + f"qualifier '{qualifier}' -> aspect '{aspect}' -> '{predicate}'",
                level="INFO",
            )
