...
Records from relevant files that are included in this ingest.
  ...
  |----------|----------|----------|
  | human_disease_knowledge_filtered.tsv | G-D association records aggregated from Medline Plus and AmyCo sources |   gene_id,	gene_name, disease_id,	disease_name,	z_score, confidence_score,	url? |
  | human_disease_textmining_filtered.tsv | All G-D association records generated by their text-mining tool |   gene_id,	gene_name,	disease_id,	disease_name,	source_db,	evidence_type?,	confidence_score |
...
Records from relevant files that are not included in the ingest.
  ...
  | human_disease_textmining_filtered.tsv | None | Taking all records regardless of score. May decide to filter lower scoring assocaitions in the future. |
...
- Revisit modleing of confidence scores/levels if/when we refactor this part of the Biolink Model.
