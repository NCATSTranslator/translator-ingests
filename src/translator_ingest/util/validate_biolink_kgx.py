...
def validate_kgx_consistency_streaming(nodes_file: Path, edges_file: Path) -> Dict[str, Any]:
    ...
    report = {
        ...
        "issues": {
            ...
            "missing_nodes": list(missing_nodes)[:100] if len(missing_nodes) <= 1000 else list(missing_nodes)[:100],  # Skip sorting for large sets
            "orphaned_nodes": list(orphaned_nodes)[:100] if len(orphaned_nodes) <= 1000 else list(orphaned_nodes)[:100],  # Skip sorting for large sets
    ...
    if orphaned_nodes:
        ...
        if len(orphaned_nodes) >= MAX_ORPHANED_NODES:
            logger.info(f"Orphaned nodes truncated at {MAX_ORPHANED_NODES}")
...
def validate_kgx_consistency(nodes_file: Path, edges_file: Path) -> Dict[str, Any]:
    ...
    report = {
        ...
        "validation_status": ValidationStatus.PASSED if validation_passed else ValidationStatus.FAILED,
        "issues": {
            "errors": errors[:1000],  # Limit to first 1000 errors
            "warnings": warnings[:1000],  # Limit to first 1000 warnings
            "missing_nodes": sorted(list(missing_nodes)) if len(missing_nodes) <= 100 else list(missing_nodes)[:100],
            "orphaned_nodes": sorted(list(orphaned_nodes)) if len(orphaned_nodes) <= 100 else list(orphaned_nodes)[:100],
        },
    }
