...
def validate_kgx_consistency(nodes_file: Path, edges_file: Path) -> bool:
    ...
    print(f"Found {len(edges)} edges referencing {len(edge_node_refs)} unique nodes")
    ...
    print(f"Nodes referenced by edges: {len(edge_node_refs)}")
    ...
    if validation_passed:
        print("VALIDATION PASSED: No missing node references")
    else:
        print("VALIDATION FAILED: Missing node references found")
...
def find_kgx_files(data_dir: Path) -> List[tuple]:
    ...
    for subdir in data_dir.iterdir():
        ...
        if len(nodes_files) > 1:
            print(f"Warning: Multiple nodes files found in {subdir}: {[f.name for f in nodes_files]}")
        if len(edges_files) > 1:
            print(f"Warning: Multiple edges files found in {subdir}: {[f.name for f in edges_files]}")
...
def validate_data_directory(data_dir: Path) -> bool:
    ...
    if not kgx_pairs:
        print(f"No KGX file pairs found in {data_dir}")
        return True

    print(f"Found {len(kgx_pairs)} KGX file pairs to validate")
